...
Jextract is a command line tool, and arg files are files containing command line arguments, separated by line-breaks.
...
```kts "argFile" "<libname>" title="Configuring an arg file in the plugin DSL"
// build.gradle.kts
jextract.libraries {
    register("<libName>") {
            whitelist {
...
```groovy "argFile" "<libname>" title="Configuring an arg file in the plugin DSL"
// build.gradle
jextract.libraries {
    register("<libName>") {
            whitelist {

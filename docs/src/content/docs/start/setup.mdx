...
:::tip
See the [latest Jextract releases](https://jdk.java.net/jextract/) on their download page.
:::

<Tabs syncKey="dsl">
<TabItem label="Kotlin">

```kts title="Minimal build script for a Java library"
// build.gradle.kts
plugins {
    `java-library`
...
```groovy title="Minimal build script for a Java library"
// build.gradle
plugins {
    id 'java-library'
    id 'de.infolektuell.jextract' version 'x.y.z'
...
```kts title="Adding a Jextract library to a Java project"
// build.gradle.kts
jextract.libraries {
...
```groovy title="Adding a Jextract library to a Java project"
// build.gradle
jextract.libraries {
  ...
  - build
    ...
    - generated
      - classes
        - jextract
          - java
          - **greeting** Generated compiled classes for the library bindings
            - …
      - sources
        - jextract
          - java
...
:::tip[Sample Project]
The [example project] demonstrates both approaches.
:::

### Automatic Loading

The code searches in the OS-specific paths for the binary files and tries to load them by their name via `system.loadLibrary`.
...
```kts {4-5} title="Configuring a library to load its binaries from the library path"
// build.gradle.kts
jextract.libraries {
    val greeting by registering {
...
```groovy {4-5} title="Configuring a library to load its binaries from the library path"
// build.gradle
jextract.libraries {
    register('greeting') {
        useSystemLoadLibrary = true
...
:::note
The plugin uses the name of the registered library configuration as library namee per default.
:::
